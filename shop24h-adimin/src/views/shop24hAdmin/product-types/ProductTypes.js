...
const ProductTypes = () => {
  ...
  useEffect(() => {
    if (typeName == "") {
      ...
      fetchAPI(`http://localhost:8000/producttype?name=${typeName}`)
        .then((data) => {
          setNoPage(Math.ceil(data.data.length / limit));
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
              ...
              <Grid item xs={9}>
                <Grid container>
                  ...
                  <Grid item xs={8} align="right">
                    <TextField onChange={(event) => setTypeName(event.target.value)} fullWidth
                ...
                {productTypes.map((productTypes, index) => {
                  return (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell>{productTypes.timeCreated}</CTableDataCell>
                      <CTableDataCell>{productTypes.name}</CTableDataCell>
                      <CTableDataCell>{productTypes.description}</CTableDataCell>
                      <CTableDataCell>
                        <ButtonGroup variant="contained">
                          <Button color="info" className="text-white" value={index} onClick={() => { onBtnEditClick(productTypes) }}>Sửa</Button>
                          <Button className="bg-danger" value={index * index} onClick={() => { onBtnDeleteClick(productTypes) }}>Xóa</Button>
                        </ButtonGroup>
                      </CTableDataCell>
                    </CTableRow>
                  )
          ...
          <ModalAddNew varRefeshPage={varRefeshPage} setOpenModalAdd={setOpenModalAdd} openModalAdd={openModalAdd}
            handleClose={handleClose} style={style} fetchAPI={fetchAPI} setVarRefeshPage={setVarRefeshPage} />
