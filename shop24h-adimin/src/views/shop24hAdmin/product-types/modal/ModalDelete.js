...
function ModalDelete({ openModalDelete, idDelete, nameDelete, handleCloseDelete, style, setVarRefeshPage, varRefeshPage }) {
    ...
    const onBtnConfirmDeleteClick = () => {
        ...
        fetch("http://localhost:8000/producttype/" + idDelete, { method: 'DELETE' })
            // .then(async response => {
    ...
    return (
        <>
            ...
            >
                <Box sx={style}>
                    <Typography mb={2} id="modal-modal-title" variant="h5" component="h2"  textAlign="center">
                        <b>Xóa Loại sản phẩm!</b>
                    </Typography>
                    <Grid container className="mt-2">
                        <Grid item xs={12} textAlign="center">
                            <h4>Bạn có thật sự muốn xóa Đông hồ loại:
